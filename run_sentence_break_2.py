...
flags.DEFINE_string(
    "bert_config_file", "/Users/trinhgiang/Downloads/multi_cased_L-12_H-768_A-12/bert_config.json",
    "The config json file corresponding to the pre-trained BERT model. "
    "This specifies the model architecture.")

flags.DEFINE_string(
    "input_file", "/Users/trinhgiang/Downloads/bert-as-language-model-master/data/lm/test.en.tsv",
    "The config json file corresponding to the pre-trained BERT model. "
    "This specifies the model architecture.")
...
flags.DEFINE_string("vocab_file", "/Users/trinhgiang/Downloads/multi_cased_L-12_H-768_A-12/vocab.txt",
                    "The vocabulary file that the BERT model was trained on.")

## Other parameters

flags.DEFINE_string(
    "init_checkpoint", "/Users/trinhgiang/Downloads/multi_cased_L-12_H-768_A-12/bert_model.ckpt",
...
def create_sequential_mask(input_tokens, input_ids, input_mask, segment_ids,
    ...
    while i < len(input_tokens) - 1:
        ...
        count_gram = 0
        tmp = i + mask_count
        while (count_gram != n_gram and tmp < (len(input_tokens) - 1)):
            m_count = 1
            while is_subtoken(input_tokens[tmp + m_count]):
                m_count += 1
            tmp += m_count
            count_gram += 1

        input_mask_new = list(input_mask)
        input_ids_new[tmp] = 102
        j = tmp
        while (input_mask_new[j + 1] != 0):
            input_mask_new[j + 1] = 0
            input_ids_new[j + 1] = 0
            j += 1
    ...
    with tf.gfile.GFile(output_file, "w") as writer:
        ...
        for word_loss in result:
            ...
            while is_subtoken(all_tokens[i]):
                ...
                sub_word_loss += word_loss[token_count_per_word]
                i += 1
            if (token_count_per_word == 0):
                sentences_word_prob.append(np.exp(-word_loss[0]))
            if (token_count_per_word != 0):
                avg_word_loss = sub_word_loss / (token_count_per_word + 1)
                sentences_word_prob.append(np.exp(-avg_word_loss))

            # end of a sentence
            if all_tokens[i] == "[SEP]":
                sentence["tokens"] = tokens
                sentence["ppl"] = float(np.exp(sentence_loss / word_count_per_sent))
                sentences_ppl.append(np.exp(sentence_loss / word_count_per_sent))
                total_sentences_word_prob.append(sentences_word_prob)
                print(sentences_word_prob)
        ...
        out = zip(sentences_ppl, total_sentences_word_prob)
        # import pickle
        # with open('./list_1.txt', 'wb') as pk:
        #     pickle.dump(out, pk)
        # return out
...
def main(_):
    ...
    output_predict_file = os.path.join(FLAGS.output_dir, "sentence_break_2.json")
    out = parse_result(result, all_tokens, output_predict_file)

    # break_index = []
    # for (ppl, probs) in out:
    #     if (ppl < 10):
    #         break_index.append(0)
    #     else:
    ...
    #                 break
    #             if (i == len(probs) - 1):
    #                 break_index.append(-1)
    #
    # data = {"predict_break_idx": break_index}
    # df = pd.DataFrame(data)
    # df.to_csv("./a.csv", encoding='utf-8', index=True)
